...
void test_read(petro::stream& bs, const std::vector<ValueType>& expected_values)
{
    // check vitals
...
TEST(test_stream, read_uint8_t)
{
    ...
    test_read<uint8_t>(bs, expected);
}

TEST(test_stream, read_int16_t)
{
    std::vector<uint8_t> data = {0xFF, 0x00, 0x37, 0xAF, 0xAF,
                                 0x37, 0x00, 0xC4, 0xC4, 0xFF};
...
TEST(test_stream, read_uint16_t)
{
    ...
    std::vector<uint16_t> expected = {0xFF00, 0x37AF, 0xAF37, 0x00C4, 0xC4FF};

    auto bs = petro::stream(data.data(), data.size());
    test_read<uint16_t>(bs, expected);
}

TEST(test_stream, read_int32_t)
{
    std::vector<uint8_t> data = {
        0xFF, 0x00, 0x37, 0xAF, 0xFF, 0x00, 0xAF, 0x37, 0x37, 0xAF, 0xC4,
        0xFF, 0xAF, 0x37, 0x00, 0xC4, 0x00, 0xC4, 0x00, 0xC4, 0xC4, 0x00,
        0xC4, 0x00, 0xC4, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC4};
    ...
    auto bs = petro::stream(data.data(), data.size());
    test_read<int32_t>(bs, expected);
}

TEST(test_stream, read_uint32_t)
{
    std::vector<uint8_t> data = {0xFF, 0x00, 0x37, 0xAF, 0xFF, 0x00, 0xAF,
                                 0x37, 0x37, 0xAF, 0xC4, 0xFF, 0xAF, 0x37,
                                 0x00, 0xC4, 0x00, 0xC4, 0x00, 0xC4, 0xC4,
                                 0x00, 0xC4, 0x00, 0xC4, 0xFF, 0xFF, 0xFF};

    std::vector<uint32_t> expected = {0xFF0037AF, 0xFF00AF37, 0x37AFC4FF,
                                      0xAF3700C4, 0x00C400C4, 0xC400C400,
                                      0xC4FFFFFF};

    auto bs = petro::stream(data.data(), data.size());
    test_read<uint32_t>(bs, expected);
}

TEST(test_stream, read_int64_t)
{
    std::vector<uint8_t> data = {
        0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x37,
        0xAF, 0x00, 0xC4, 0xF2, 0xC4, 0xFF, 0xF5, 0xAF, 0x37, 0xC4, 0xFF,
        0x33, 0xA8, 0x37, 0xAF, 0xC4, 0xFF, 0x04, 0x11, 0xD3, 0xD3, 0xAF,
        0x37, 0xF5, 0xC4, 0x00, 0xC4, 0xF2, 0xC4, 0x00, 0xC4, 0x00, 0xC4,
        0x33, 0xA8, 0xF5, 0x00, 0xC4, 0x00, 0xC4, 0xD3, 0x00, 0xC4, 0xC4,
        0xC4, 0xC4, 0xFF, 0xFF, 0xFF, 0x33, 0xA8, 0x00, 0x34};
...
TEST(test_stream, read_uint64_t)
{
    ...
    std::vector<uint64_t> expected = {0xFFFFFFFF00000000, 0xFF0037AF00C4F2C4,
                                      0xFFF5AF37C4FF33A8, 0x37AFC4FF0411D3D3,
                                      0xAF37F5C400C4F2C4, 0x00C400C433A8F500,
                                      0xC400C4D300C4C4C4, 0xC4FFFFFF33A80034};

    auto bs = petro::stream(data.data(), data.size());
    test_read<uint64_t>(bs, expected);
}
...
TEST(test_stream, read_fixed_point_1616)
{
    ...
    std::vector<double> expected = {0.0, 1 << 16};
...
TEST(test_stream, read_fixed_point_88)
{
    ...
    // is thought to be correct.
    // The point of this test is to get notified if the result changes.
    std::vector<uint8_t> data = {0x00, 0x00, 0xFF, 0xFF};
...
{
    std::vector<uint8_t> data = {0x55, 0xC4, 0x1C, 0xB2};

    std::vector<std::string> expected = {"und", "ger"};
...
{
    std::vector<uint8_t> data = {0x7C, 0x25, 0xB0, 0x80, 0x00, 0x00,
                                 0x00, 0x00, 0xCA, 0x8C, 0xBA, 0x6E,
                                 0xCA, 0x8C, 0xBA, 0x6F};

    std::vector<std::string> expected = {
        "1970-01-01 00:00:00", "before 1970-01-01 00:00:00",
        "2011-09-07 06:06:38", "2011-09-07 06:06:39"};
...
TEST(test_stream, read_time64)
{
    std::vector<uint8_t> data = {
        ...
        0x00, 0x7C, 0x25, 0xB0, 0x80, 0x00, 0x00, 0x00, 0x00, 0xCA, 0x8C,
        0xBA, 0x6E, 0x00, 0x00, 0x00, 0x00, 0xCA, 0x8C, 0xBA, 0x6F};
