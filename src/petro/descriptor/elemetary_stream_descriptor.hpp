...
public:
    ...
    void parse_descriptor_content(std::error_code& error) override
    {
        ...
        m_bs.read<uint16_t>(m_es_id, error);
        if (error)
            return;

        uint8_t flags = 0;
        m_bs.read<uint8_t>(flags, error);
        if (error)
            return;
        ...
        {
            m_bs.read<uint16_t>(m_depends_on_esid, error);
            if (error)
                return;
        }
        ...
        if (url_flag)
        {
            ...
            m_bs.read<uint8_t>(url_length, error);
            if (error)
                return;
        ...
        if (ocr_stream_flag)
        {
            m_bs.read<uint16_t>(m_ocr_es_id, error);
            if (error)
                return;
        }
